...
const ImportMgmt = ({ navigation }) => {
    const [importDate, setImportDate] = useState('2023-02-05'); // 초기값 (이미지 기준)
    ...
    const handleSearch = async () => {
        try {
            setLoading(true);
            const response = await commonAPI.selectImportMgmt({ importDate });

            // 임시 데이터 (이미지 기반 샘플)
            const mockResult = [
                { id: '1', consignorId: 'HN', containerNumber: 'CKCO1234567', vehicleNo: '4222', importTime: '15:24' },
                { id: '2', consignorId: 'DDS', containerNumber: 'CKCO7654321', vehicleNo: '3331', importTime: '14:30' },
            ];
    ...
    const handleSave = async () => {
        ...
        try {
            setLoading(true);
            await commonAPI.updateImportMgmt(updatedData);
    ...
    const renderItem = ({ item }) => (
        <View style={styles.tableRow}>
            ...
            <View style={[styles.cell, { flex: 2, paddingHorizontal: 5 }]}>
                <TextInput
            ...
            <View style={[styles.cell, { flex: 2, paddingHorizontal: 5 }]}>
                <TextInput
                    ...
                    onChangeText={(val) => handleEdit(item.id, 'importTime', val)}
                />
            </View>
        </View>
    );
        ...
        <View style={styles.container}>
            ...
            <View style={styles.content}>
                <View style={styles.tableHeader}>
                    ...
                    <View style={[styles.headerCell, { flex: 2 }]}>
                        <Text style={styles.headerCellText}>반입시간</Text>
                    </View>
                </View>
