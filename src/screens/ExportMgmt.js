...
const ExportMgmt = ({ navigation }) => {
    ...
    const handleSearch = async () => {
        try {
            setLoading(true);
            const response = await commonAPI.selectExportMgmtList(exportDate);
    ...
    const onDateChange = (event, selectedDate) => {
        ...
        if (event.type === 'set' && selectedDate) {
            ...
            if (selectedRowId === 'top') {
                ...
                handleEdit(selectedRowId, 'shortBringOutDate', formattedDate);
            }
        }
    };
    ...
    const handleSave = async () => {
        ...
        // 시간 4자리 유효성 검사
        const invalidTimeItem = selectedItems.find(item => item.bringOutTime && item.bringOutTime.length !== 4);
        if (invalidTimeItem) {
        ...
        try {
            ...
            await commonAPI.updateExportMgmtList(updatedData);
        ...
        <View style={styles.tableRow}>
            ...
            <View style={[styles.cell, { flex: 2 }]}>
                <TextInput
                    ...
                    value={item.bringOutRegistrationNumber}
                    maxLength={4}
                    onChangeText={(val) => handleEdit(item.id, 'bringOutRegistrationNumber', val)}
            ...
            <View style={[styles.cell, { flex: 2.5, paddingHorizontal: 3, paddingVertical: 3 }]}>
                <View style={{ flex: 1, width: '100%', borderRadius: 4, overflow: 'hidden', backgroundColor: '#E2E8F0' }}>
                    <TouchableOpacity
                        ...
                        onPress={() => openDatePicker(item.id, item.shortBringOutDate)}
                    >
                        <Text style={styles.cellText}>{item.shortBringOutDate || '-'}</Text>
                    </TouchableOpacity>
                    <View style={styles.subCell}>
                        <TextInput
                            style={[styles.cellInput, { backgroundColor: 'transparent', paddingVertical: 0, height: '100%' }]}
                            value={item.bringOutTime}
                            maxLength={4}
                            onChangeText={(val) => handleTimeChange(item.id, 'bringOutTime', val)}
                            keyboardType="numeric"
                        />
                    </View>
                </View>
        ...
        <View style={styles.container}>
            ...
            <View style={styles.filterArea}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => openDatePicker('top', exportDate)}
                >
                    <Text style={{ fontSize: 16, color: '#333' }}>{exportDate}</Text>
                </TouchableOpacity>
